...
def _inject_styles() -> None:
    st.markdown(
        ...
        *, *::before, *::after { box-sizing:border-box; }
        ...
        .particle  { position:absolute; width:6px; height:6px; border-radius:50%; background:rgba(74,220,74,.35); animation:particleDrift linear infinite; }
        ...
        .hero-wrap {
            position:relative; border-radius:24px; padding:3rem 2.5rem 2.5rem; margin-bottom:2rem;
            overflow:hidden;
        ...
        .hero-orb  { position:absolute; right:2.5rem; top:50%; transform:translateY(-50%); width:110px; height:110px; }
        .orb-core  { width:70px; height:70px; border-radius:50%; background:radial-gradient(circle,rgba(74,220,74,.35) 0%,rgba(20,80,20,.15) 70%,transparent 100%); position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); animation:floatUp 4s ease-in-out infinite; display:flex; align-items:center; justify-content:center; font-size:2rem; }
        ...
        .orb-dot   { width:10px; height:10px; border-radius:50%; background:#4adc4a; box-shadow:0 0 12px rgba(74,220,74,.7); animation:orbit 4s linear infinite; position:absolute; top:50%; left:50%; margin-top:-5px; margin-left:-5px; }
        .hero-badge { display:inline-block; background:rgba(74,220,74,.12); border:1px solid rgba(74,220,74,.3); border-radius:99px; padding:.25rem .8rem; font-size:.7rem; letter-spacing:2px; text-transform:uppercase; color:#4adc4a; margin-bottom:.8rem; animation:fadeSlideLeft .6s ease both; }
        .hero-title { font-weight:900; font-size:clamp(2rem,5vw,3.5rem); line-height:1.05; letter-spacing:-1px; color:#fff; text-shadow:0 0 40px rgba(74,220,74,.25); animation:fadeSlideIn .9s ease both; margin-bottom:.6rem; }
        .hero-title span { background:linear-gradient(135deg,#4adc4a,#a8f5a8,#4adc4a); background-size:200% 100%; -webkit-background-clip:text; -webkit-text-fill-color:transparent; animation:gradientShift 3s ease infinite; }
        ...
        .scard { background:rgba(12,28,14,.8); border:1px solid rgba(74,220,74,.18); border-radius:20px; padding:1.8rem 1.6rem 1.4rem; margin-bottom:1.5rem; backdrop-filter:blur(12px); animation:fadeSlideIn .6s ease both, borderDance 5s ease-in-out infinite; position:relative; overflow:hidden; }
        .scard::after { content:''; position:absolute; top:-1px; left:20%; right:20%; height:2px; background:linear-gradient(90deg,transparent,rgba(74,220,74,.5),transparent); border-radius:99px; }
        .scard-title { font-weight:700; font-size:.7rem; letter-spacing:3px; text-transform:uppercase; color:#4adc4a; margin-bottom:1.2rem; display:flex; align-items:center; gap:.5rem; }
        .scard-title::after { content:''; flex:1; height:1px; background:linear-gradient(90deg,rgba(74,220,74,.3),transparent); }

        .stNumberInput label, .stSelectbox label, .stSlider label, .stTextInput label { color:#90e890 !important; font-size:.83rem !important; font-weight:500 !important; }
        .stNumberInput input, .stTextInput input { background:rgba(6,18,8,.9) !important; border:1px solid rgba(74,220,74,.25) !important; border-radius:12px !important; color:#e0f5e0 !important; font-family:'Outfit',sans-serif !important; font-size:.9rem !important; transition:border-color .25s,box-shadow .25s !important; }
        .stNumberInput input:focus, .stTextInput input:focus { border-color:#4adc4a !important; box-shadow:0 0 0 3px rgba(74,220,74,.18) !important; }
        div[data-baseweb="select"] > div { background:rgba(6,18,8,.9) !important; border:1px solid rgba(74,220,74,.25) !important; border-radius:12px !important; color:#e0f5e0 !important; transition:border-color .25s !important; }
        ...
        .mbox { background:linear-gradient(145deg,rgba(16,42,18,.9),rgba(8,22,10,.9)); border:1px solid rgba(74,220,74,.2); border-radius:14px; padding:1rem .8rem; text-align:center; transition:transform .2s,border-color .2s,box-shadow .2s; animation:scaleIn .5s ease both; }
        ...
        .stButton button[kind="primary"] { position:relative; overflow:hidden; background:linear-gradient(135deg,#1d6b1d,#4adc4a,#1d6b1d) !important; background-size:200% 100% !important; border:none !important; border-radius:14px !important; color:#fff !important; font-family:'Outfit',sans-serif !important; font-weight:700 !important; font-size:1rem !important; padding:.75rem 2rem !important; width:100%; box-shadow:0 4px 24px rgba(74,220,74,.35) !important; transition:background-position .4s,transform .2s,box-shadow .2s !important; animation:pulseGlow 3s ease-in-out infinite !important; }
        .stButton button[kind="primary"]::before { content:''; position:absolute; inset:0; background:linear-gradient(90deg,transparent 0%,rgba(255,255,255,.15) 50%,transparent 100%); background-size:200% 100%; animation:shimmer 2s linear infinite; }
        .stButton button[kind="primary"]:hover { background-position:100% 0 !important; transform:translateY(-3px) !important; box-shadow:0 10px 36px rgba(74,220,74,.5) !important; }
        .stButton button[kind="secondary"] { background:rgba(74,220,74,.07) !important; border:1px solid rgba(74,220,74,.3) !important; border-radius:12px !important; color:#90e890 !important; font-family:'Outfit',sans-serif !important; font-weight:500 !important; width:100%; transition:all .25s !important; }
        .stButton button[kind="secondary"]:hover { background:rgba(74,220,74,.15) !important; border-color:#4adc4a !important; transform:translateY(-2px) !important; box-shadow:0 4px 18px rgba(74,220,74,.2) !important; }

        [data-testid="metric-container"] { background:rgba(12,28,14,.85) !important; border:1px solid rgba(74,220,74,.2) !important; border-radius:14px !important; padding:.8rem 1rem !important; animation:scaleIn .5s ease both; transition:border-color .2s,box-shadow .2s !important; }
        ...
        .crop-card { display:flex; align-items:center; gap:.8rem; background:rgba(10,25,12,.85); border:1px solid rgba(74,220,74,.15); border-radius:16px; padding:1rem 1.2rem; margin-bottom:.7rem; transition:transform .25s,border-color .25s,box-shadow .25s; animation:fadeSlideLeft .5s ease both; }
...
def _render_hero() -> None:
    st.markdown(
        ...
        <div class="hero-wrap">
            ...
            <div class="hero-sub">Precision crop recommendations · Soil NPK + pH · Live weather</div>
...
def main() -> None:
    ...
    if submitted:
        ...
        if weather:
            st.markdown('<div class="scard"><div class="scard-title">🌤 Live Weather Data Used</div>', unsafe_allow_html=True)
            ...
            w3.metric("🌧 Rainfall",    f"{rainfall    if rainfall    is not None else '0'} mm")
